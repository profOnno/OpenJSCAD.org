...
const { hasPointInside } = require("@jscad/csg/src/core/utils/cagValidation");
...
//  console.log("Worker"); // TODO DONT USE WORKER AS VARIABLE
...
const cagFromString = (font, str, size, options) => { // TODO add other params
  ...
  if (typeof str === 'undefined' || str.length < 1) {
    //console.log("aargh");
    throw new Error("cagFromString needs string as 2nd parameter");
  }

  const fp = font.getPath(str, 0, 0, size);  

  let csgPath = []; // array of csg paths
  let np; // nexPath
//      Worker.handle.postMessage({ cmd: 'status', status: "gonna make path.." });
  for (let i = 0; i < fp.commands.length; i += 1) {
    const cmd = fp.commands[i];
    switch(cmd.type) {
      case "M": // move asume start
...
//        console.log("got close");
      ...
      case "Q": // quadrathingy TODO
        np = np.appendBezier([[cmd.x1, -cmd.y1],[cmd.x, -cmd.y]]);
        break;
    }
  }
//  console.log("csgPath:");
//  console.log(csgPath);
  ...
  return _makeCAGS(csgPath); 
};

// TODO make a tree? mabe only needed with glyph
// there ar no letters with solid -> hole -> solid in hole?
// ghost from pac man would have solid in his eyes
const _makeCAGS = (path_array, options) => {
  let fontCAGS = []
//  options = Object.assign({}, { grow: false }, options);
//  console.log("options:");
//  console.log(options);

  let volume = path_array[0].innerToCAG();
 // if (options.grow) { volume = volume.expand(options.grow); }
  for (let i = 1; i < path_array.length; i++) {
    if( pointInCAG(volume, path_array[i].points[0] )) { // TODO check if this is always valid use windings oid
//      console.log("point in CAG");
      let nvolume = path_array[i].innerToCAG();
//      if (options.grow) { nvolume = nvolume.contract(options.grow); }
      volume = volume.subtract(nvolume);
    } else {
      fontCAGS.push(volume);
      volume = path_array[i].innerToCAG();
//      if (options.grow) { volume = volume.expand(options.grow); }
    }
  }
  fontCAGS.push(volume);
